...
// const API_URL = 'http://localhost:8000' // РАСКОММЕНТИРУЙ ЭТУ СТРОКУ И ЗАКОММЕНТИРУЙ ВЕРХНЮЮ, ЕСЛИ ТЕСТИРУЕШЬ БЭКЕНД ЛОКАЛЬНО НА КОМПЕ
...
function App() {
  ...
  // --- ОБРАБОТЧИКИ (ТЕПЕРЬ С TRY-CATCH ДЛЯ ЗАЩИТЫ ОТ ПАДЕНИЙ) ---
  const handleAddIdea = async (e) => {
    ...
    try {
      ...
      alert("❌ Ошибка: Сервер еще не обновился на Render. Подожди пару минут или проверь, залил ли ты новый main.py!");
    }
  }
  ...
  // 🔥 НОВАЯ СУПЕР-ФУНКЦИЯ: Превращение Идеи в реальный Квест
  const turnIdeaIntoQuest = async (idea) => {
    try {
      // 1. Создаем квест из идеи
      await axios.post(`${API_URL}/add_quest`, {
        title: idea.text,
        description: "💡 Сгенерировано из Бэклога Идей",
        xp: 25,
        gold: 20,
        category: "🎬 Личный Бренд", // Автоматически закидываем в бренд
        subcategory: "Сценарии",
        requires_id: "",
        is_daily: false
      });
      // 2. Удаляем идею из бэклога
      await axios.delete(`${API_URL}/delete_idea/${idea.id}`);
      playRetroSound('coin');
      fetchData();
      setActiveTab('play'); // Перекидываем на экран квестов
    } catch (error) {
      alert("Ошибка при переносе идеи!");
    }
  }
  ...
  return (
    <div className="relative min-h-screen bg-[#040914] text-slate-200 flex flex-col items-center py-8 px-4 font-sans pb-28 overflow-x-hidden selection:bg-teal-500/30">
      ...
      {/* ИДЕИ С НОВОЙ ФУНКЦИЕЙ КОНВЕРТАЦИИ */}
      {activeTab === 'ideas' && (
        <div className="w-full max-w-md space-y-6 relative z-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
          <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-[2rem] p-7 shadow-[0_8px_32px_0_rgba(0,0,0,0.3)]">
                ...
                ideas.map(idea => (
                  <div key={idea.id} className="group relative flex flex-col justify-center bg-white/5 border border-white/5 p-4 rounded-xl hover:bg-white/10 transition-colors">
                    ...
                    <div className="flex gap-2 w-full">
                      {/* НОВАЯ КНОПКА КОНВЕРТАЦИИ В КВЕСТ */}
      ...
      {/* ОСТАЛЬНЫЕ ВКЛАДКИ (МАГАЗИН И БАЗА) */}
      {activeTab === 'shop' && (
          ...
          <div className="bg-white/5 backdrop-blur-xl rounded-[2rem] p-7 border border-white/10">
            ...
            <form onSubmit={handleAddQuest}>
              ...
              <div className="grid grid-cols-2 gap-4 mb-5">
                <input type="number" placeholder="XP" required min="1" value={newXp} onChange={(e) => setNewXp(e.target.value)} className="w-full bg-black/20 text-teal-300 rounded-xl px-4 py-3.5 border border-white/10 text-sm" />
                <input type="number" placeholder="Gold" required min="1" value={newGold} onChange={(e) => setNewGold(e.target.value)} className="w-full bg-black/20 text-amber-300 rounded-xl px-4 py-3.5 border border-white/10 text-sm" />
              </div>
